...
[ -e "${rootvolume}/data/SSDVN/SSDVN-API/license.dat" ] || { log-error "License does not exist"; exit 1; }
...
altso=$(find "${rootvolume}/" -maxdepth 1 -mindepth 1  -type f -iname "alt*linux-gnu.so" | sort -r | head  -n 1)
[[ -e "${altso}" ]] && {
...
#-e P=1
dockerruncmd=$(cat <<__
nvidia-docker run -idt -e CUDA_VISIBLE_DEVICES=0 -e TZ=Asia/Ho_Chi_Minh -e LD_LIBRARY_PATH=/usr/local/cuda-11.7/lib64 --name $CONNAME --hostname $CONNAME \
-v "${rootvolume}/data/SSDVN/SSDVN-SOC/license.dat/:/server/modules/module_so/soc/core/authorization/license.dat" \
-v "${rootvolume}/data/SSDVN/SSDVN-API/license.dat/:/server/api/authorization/license.dat" \
...
cnt=0
while :; do
    respone="$(curl -s --location --request POST '172.172.0.50/vnhff/v1/recognition' \
