...
[ -e "${rootvolume}/setup/PROJP2-License/PROJP2-API/license.dat" ] || { log-error "License does not exist"; exit 1; }
...
-v "${rootvolume}/setup/pb-1.0.2-cpu-stg/:/server/database/ocr/ocr_templates/db/cloud" \
-v "${rootvolume}/setup/api.ini/:/server/api/configs/api.ini" \
-v "${rootvolume}/setup/black_list.txt/:/server/api/authorization/black_list/black_list.txt" \
-v "${rootvolume}/setup/PROJP2-License/PROJP2-API/license.dat/:/server/api/authorization/license.dat" \
-v "${rootvolume}/setup/PROJP2-License/PROJP2-SOC/license.dat/:/server/modules/module_so/soc/core/authorization/license.dat" \
-v "${rootvolume}/setup/alt.cpython-36m-x86_64-linux-gnu.so/:/server/database/ocr/alt.cpython-36m-x86_64-linux-gnu.so" \
-v "${rootvolume}/setup/table_reconstruction_19092022_0.9.0.0/table_reconstruction/:/server/modules/module_so/table_reconstruction" \
...
cnt=0
while :; do
    respone="$(curl -s --location --request POST 'http://172.172.0.10/vnhff/v1/recognition' \
