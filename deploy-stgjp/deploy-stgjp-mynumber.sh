...
[ -e "${rootvolume}/apikey/keys.txt" ] || touch "${rootvolume}/apikey/keys.txt"

dockerruncmd=$(
  cat <<__
docker run -idt --name $CONNAME --hostname $CONNAME \
-v "${rootvolume}/apikey/keys.txt:/server/api/apikey/keys.txt" \
-v "${rootvolume}/config/api.ini:/server/api/configs/api.ini" \
-v "${rootvolume}/templates:/server/database/ocr/ocr_templates/db" \
-v "${rootvolume}/soc:/server/module_so/soc/" \
