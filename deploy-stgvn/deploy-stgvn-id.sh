...
dockerruncmd=$(
  cat <<__
docker run  -idt -e P=1 --name $CONNAME --hostname $CONNAME --restart always --privileged --ulimit core=0 -e TZ=Asia/Ho_Chi_Minh \
...
-v "${rootvolume}/data/config/API/config.ini:/workspace/config/config.ini" \
...
while :; do
  respone="$(curl -s --location --request POST 'https://stgapi.smartocr.vn/idfull/v1/recognition' \
    --header 'api-key: gAAAAABkDsp7xiSLPxhvikD105Bg-YJxl0vhyCSQ1zp-RHE6SMqoYhSc52iU-nzGu74Oe4_KpdxIjvL61FMlMH-1AN2xE_3TjKsWA1TqRxWjt_jKcHHKwSc=' \
