...
[ -e "${rootvolume}/setup/license/key.dat" ] || {
...
{ [ ! -e "${rootvolume}/setup/config/db_config/mongo.ini" ] || [[ ! $(cat "${rootvolume}/setup/config/db_config/mongo.ini") ]]; } && {
  [[ $mongoconf ]] && {
    touch "${rootvolume}/setup/config/db_config/mongo.ini"
    echo "$mongoconf" >"${rootvolume}/setup/config/db_config/mongo.ini"
  } || {
    log-error "Lack of configuration MongoDB: '${rootvolume}/setup/config/db_config/mongo.ini'"
    exit 1
  }
}
...
docker run -idt -e P=1 --name $CONNAME --hostname $CONNAME --restart always -e TZ=Asia/Ho_Chi_Minh \
   -v "${rootvolume}/setup/license/key.dat:/workspace/key.dat" \
   -v "${rootvolume}/setup/config/api/config.ini:/workspace/setting/api_config/config.ini" \
   -v "${rootvolume}/setup/config/service/config_compose.py:/workspace/config_compose.py" \
   -v "${rootvolume}/setup/config/db_config/:/workspace/setting/db_config/" \
   -v "${rootvolume}/setup/config/trust/config_url.ini:/workspace/face_recog/send_request/config_url.ini" \
   -v "${rootvolume}/setup/template/cloud/:/workspace/setting/templates/cloud/" \
   -v "${rootvolume}/../app_log:/workspace/logs/app_log" \
...
#[ -e "${rootvolume}/config_url.ini" ] && {
#  log-warning "Update black trust config..."
#  docker cp "${rootvolume}/config_url.ini" "${CONNAME}:/workspace/face_recog/send_request/config_url.ini"
#}
...
cnt=0
while :; do
  respone="$(curl -s --location --request POST 'https://stgapi.smartocr.vn/face/v1/recognition' \
